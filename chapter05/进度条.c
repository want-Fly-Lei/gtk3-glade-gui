...
int main() {
    ...
    // 废弃 api
    // GtkVBox *vbox = GTK_VBOX(gtk_vbox_new(TRUE, 5));
    // Vbox
    GtkBox * vbox = GTK_BOX(gtk_box_new(GTK_ORIENTATION_VERTICAL, 5));

    bt1 = GTK_BUTTON(gtk_button_new_with_label("快进"));
    bt2 = GTK_BUTTON(gtk_button_new_with_label("倒退"));
    bt3 = GTK_BUTTON(gtk_button_new_with_label("切换方向"));
    ...
    gtk_container_add(GTK_CONTAINER(vbox), GTK_WIDGET(bar));
    gtk_container_add(GTK_CONTAINER(vbox), GTK_WIDGET(bt1));
    g_signal_connect(bt1, "clicked", G_CALLBACK(bt1Click), bar);
    gtk_container_add(GTK_CONTAINER(vbox), GTK_WIDGET(bt2));
    g_signal_connect(bt2, "clicked", G_CALLBACK(bt2Click), bar);
    gtk_container_add(GTK_CONTAINER(vbox), GTK_WIDGET(bt3));
    g_signal_connect(bt3, "clicked", G_CALLBACK(bt3Click), bar);
