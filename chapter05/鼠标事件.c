...
gboolean windowMove(GtkWidget *widget, GdkEventMotion *event, gpointer user_data) {
    // 这里的移动时拖拉这window才算
    printf("mouses location: x = %.2lf, y = %.2lf\n", event->x, event->y);
}
...
int main(int argc, char *argv[]) {
    ...
    gtk_widget_add_events(GTK_WIDGET(window), GDK_BUTTON_PRESS_MASK | GDK_BUTTON_MOTION_MASK);

    // 设置信号,鼠标点击事件，gtk event需要回调函数传入三个参数才行
    g_signal_connect(window, "button-press-event", G_CALLBACK(windowClick), NULL);
    // 鼠标移动事件
    g_signal_connect(window, "motion-notify-event", G_CALLBACK(windowMove), NULL);
